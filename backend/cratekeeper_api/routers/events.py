...
from cratekeeper_api.services.event_aggregation import (
    build_event_out,
    build_event_outs,
)
...
def _to_out(ev: Event, count: int = 0) -> EventOut:
    """Basic EventOut for create/update responses (no aggregates)."""
    return EventOut(
        id=ev.id,
        name=ev.name,
        slug=ev.slug,
...
async def list_events(db: Session = Depends(get_db)) -> list[EventOut]:
    """Return every event with the dashboard aggregates (CRATE-1)."""
    events = (
        db.execute(select(Event).order_by(Event.created_at.desc()))
        .scalars()
        .all()
    )
    return build_event_outs(db, list(events))
