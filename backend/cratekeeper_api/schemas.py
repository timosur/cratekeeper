...
class EventOut(BaseModel):
    ...
    # --- Aggregated dashboard fields (CRATE-1) -----------------------------
    # Populated by the read endpoints (`GET /events`, `GET /events/{id}`).
    # Create/update endpoints return defaults; clients should re-fetch via
    # GET if they need the aggregates.
    matched_count: int = 0
    tagged_count: int = 0
    current_step_index: int = 0
    current_step_label: str = "Fetch"
    total_steps: int = 11
    is_stale: bool = False
...
class JobOut(BaseModel):
    ...
    created_at: datetime
    # Populated only by the Events list endpoints (CRATE-1) when status is "queued".
    queue_position: int | None = None


# Resolve the forward reference on EventOut.active_job once JobOut exists.
