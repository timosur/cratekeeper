...
async def run(ctx: JobContext) -> dict:
    ...
    if not dry_run and consumed_ids:
        with ctx.db_session() as db:
            ...
            # Clear `tagged_at` on the restored tracks so the dashboard's tagged_count reflects reality.
            restored_spotify_ids = [
                row[1] for row in rows if row[0] in consumed_ids
            ]
            if restored_spotify_ids:
                db.query(EventTrack).filter(
                    EventTrack.event_id == event_id,
                    EventTrack.spotify_id.in_(restored_spotify_ids),
                ).update({"tagged_at": None}, synchronize_session=False)
