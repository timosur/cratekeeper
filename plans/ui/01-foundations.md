...
1. **Architecture and foundations**: define bounded contexts (Events, Tracks, Workflow Jobs, Playlist Integrations, Settings). Map CLI commands in [cli.py](../../cratekeeper-api/cratekeeper/cli.py) to backend services. Decide per module whether to invoke directly or wrap in an adapter.

2. **Data model design**: extend the existing PostgreSQL schema. [local_scanner.py](../../cratekeeper-api/cratekeeper/local_scanner.py) owns the canonical `tracks` table — do not duplicate. Add new tables:
   ...
   Introduce Alembic for migrations. Define a CLI/web coexistence contract so both read/write without corruption. Map to existing [models.py](../../cratekeeper-api/cratekeeper/models.py) dataclasses.
